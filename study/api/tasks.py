...
@task(name="request_order_payment")
def request_order_payment(order_id):
    ...
    if payment_request.status == 200:
        response_data = json.loads(payment_request)
        payment_response_data = PaymentResponse(**response_data)

        order = Order.objects.get(id=order_id)
        order.order_status = payment_response_data.status
        order.save()

        update_user_membership.delay()
        """sends an email when user membership is created successfully"""
